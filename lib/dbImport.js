...
var importPlatformAssoc = function (dbConnection, assoc, film, cb) {
    if ((!film[assoc] || !film[assoc].length)) {
        cb();
        return;
    }
    ...
    if (film[assoc]) {
        ...
        film[assoc].forEach(function(platformId, index, array) {
            insertAssoc(dbConnection, assoc, film.id, platformId, index, function() {
                itemsProcessed++;
        ...
        assoc.forEach(function(assocName) {
            importAssoc(dbConnection, assocName, film, function() {
                ...
                if(assocProcessed == assoc.length) {
                    clearFilmPlatforms(dbConnection, film.id, function() {
                        importPlatformAssoc(dbConnection, 'platform', film, cb);
                    })
                }
            });
        });
...
var clearNewFilmsInPlatform = function (platform, dbConnection, cb) {
    const query = 'DELETE FROM newFilmsInPlatform WHERE platform = ' + mysql.escape(platform);

    global.log.info('***** Truncating newFilmsInPlatform');
...
var clearFilmPlatforms = function (dbConnection, idFilm, cb) {
    const query = 'DELETE FROM assocFilmPlatform WHERE idFilm = ' + idFilm;
...
var insertNewFilmsInPlatform = function (platform, dbConnection, film, cb) {
    const query = 'INSERT INTO newFilmsInPlatform VALUES(' +
      film.id + ',' +
    ...
    global.log.info('***** Inserting into newFilmsInPlatform: ' + film.id + '|' + platform + '|' + film.releaseDate);
...
var getAllFilmIds = function (dbConnection, cb) {
    const query = 'SELECT idFilm FROM film ORDER BY idFilm ASC';
    ...
    dbConnection.query(query, function (err, results) {
        ...
        const filmsInDb = results.map(result => result.idFilm);
        const missingIdsToCrawl = missingIds(filmsInDb);

        cb(missingIdsToCrawl);
    });
};

var getAllFilmIds_0 = function (dbConnection, cb) {
    const query = 'SELECT idFilm FROM film_tmp ORDER BY idFilm ASC';
...
module.exports = {
    ...
    clearNewFilmsInPlatform: clearNewFilmsInPlatform,
    insertNewFilmsInPlatform: insertNewFilmsInPlatform,
    ...
    getAllFilmIds: getAllFilmIds,
    importPlatformAssoc: importPlatformAssoc,
};
