...
var insertAssoc = function(dbConnection, assoc, idFilm, idAssoc) {
...
var importAssoc = function (dbConnection, assoc, film) {
    if (film[assoc]) {
                ...
                if (err) {
                    if (err.code == 'ER_DUP_ENTRY') {
                        ...
                        dbConnection.query(querySelect, function (err, resultSelect) {
                            ...
                            insertAssoc(dbConnection, assoc, film.id, resultSelect[0]['id' + assoc]);
                        });
                    } else {
                    ...
                    insertAssoc(dbConnection, assoc, film.id, resultInsert.insertId);
                }
            });
        });
    }
...
var importFilm = function (dbConnection, film, cb) {
    ...
    dbConnection.query(queryNewFilm, function (err) {
        ...
        assoc.forEach(function(assoc) {
            importAssoc(dbConnection, assoc, film);
        });
