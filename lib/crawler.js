...
const proxyRequest = require('gimmeproxy-request').request;

setup({
    api_key: '',
    query: 'supportsHttps=true&anonymityLevel=1&maxCheckPeriod=600', // additional gimmeproxy query parameters
...
var getNumPagesOfFilmsStartingWithChar = function (char, callback) {
    ...
    proxyRequest(url, {timeout: 10000}, function (error, response, body) {
...
var loadPopularFilmsPage = function(endpoint, callback) {
    ...
    request.get(getRequestOptions(url), function (error, response, body) {
        if (error || response.statusCode != 200 || captchaTriggered(body)) {
            ...
            filmsDom.each(function() {
                var film = {
                    id: $(this).attr('href').match('/es/film(.*).html')[1],
                    position: $(this).parent().parent().parent().parent().find('.position').text()
                };
