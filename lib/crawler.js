  ...
  getProxyRequestOptions(url, true, function (requestOptions) {
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        ...
        filmsDom.each(function () {
          var dateNumbers = $(this).prevAll('.rdate-cat').first().attr('id').split('-'),
            // dateNumbers comes in format 'date-2024-11-25
            year = dateNumbers[1],
            month = dateNumbers[2],
            day = dateNumbers[3],
...
var processProReviews = function (htmlDom) {
  ...
  htmlDom('#pro-reviews .pro-review').each(function () {
    let reviewBody = htmlDom(this).find('[itemprop=reviewBody]').text().trim(),
      reviewAuthorElement = htmlDom(this).find('.pro-crit-med a[itemprop=name]'),
      reviewAuthor = reviewAuthorElement.text().trim(),
      reviewMedium = htmlDom(this).find('.pro-crit-med strong a').text().trim(),
      author = reviewAuthor + ': ' + reviewMedium,
      trendElement = htmlDom(this).find('.pro-crit-med i'),
      trendRaw = trendElement.attr('class'),
      trend;
    ...
    review.push({'review': reviewBody, 'author': author, 'trend': trend});
  });
    ...
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        ...
        var film = {
          ...
          title: $(body).find('#main-title span').first().text().trim(),
