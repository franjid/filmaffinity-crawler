...
var handleFailedRequest = function(originalFunName, error, response) {
    console.log('Some error happened with request in ' + originalFunName);
    console.log(error);
    if (response && response.statusCode) {
        console.log(response.statusCode + "\n");
    }

    console.log('Refreshing Tor IP...');
    try {
        /*exec('echo -e \'AUTHENTICATE ""\r\nsignal NEWNYM\r\nQUIT\' | nc localhost 9051', function (error, out, err) {
            console.log(error);
        });*/
    } catch (e) {
        console.log("Catched exec error: " + e);
    }
    console.log("\n");
};
