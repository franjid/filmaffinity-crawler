...
var getProxyRequestOptions = function (url, callback) {
  if (Math.random() >= 0.5) {
    global.log.info('Using Tor');
    callback(getRequestOptions(url));
    return;
  }
  ...
  request.get(requestOption, function (error, response, body) {
    if (error || response.statusCode != 200) {
      ...
      getProxyRequestOptions(url, callback);
    } else {
      let response = JSON.parse(body);

      let requestOptions = getRequestOptions(url);
      requestOptions.proxy = response.protocol + '://' + response.ip + ':' + response.port;
...
var getNumPagesOfFilmsStartingWithChar = function (char, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
...
var loadFilmsPage = function (endpoint, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        handleFailedRequest('loadFilmsPage', error, response);
        loadFilmsPage(endpoint, callback); // Keep trying until we get something
      } else {
        var $ = cheerio.load(body),
...
var loadTopFilmsPage = function (page, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    ...
    request.post(requestOptions, function (error, response, body) {
      ...
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        handleFailedRequest('loadTopFilmsPage', error, response);
        loadTopFilmsPage(page, callback); // Keep trying until we get something
      } else {
        var $ = cheerio.load(body),
          filmsDom = $(body).find('.mc-poster a'),
          films = [];
...
var loadPopularFilmsPage = function (endpoint, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        handleFailedRequest('loadPopularFilmsPage', error, response);
        loadPopularFilmsPage(endpoint, callback); // Keep trying until we get something
      } else {
        var $ = cheerio.load(body),
...
var loadFilmsInTheatresPage = function (endpoint, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        handleFailedRequest('loadFilmsInTheatresPage', error, response);
        loadFilmsInTheatresPage(endpoint, callback); // Keep trying until we get something
      } else {
        var $ = cheerio.load(body);
...
var loadUserLastFilmsRated = function (userId, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        handleFailedRequest('loadUserLastFilmsRated', error, response);
        loadUserLastFilmsRated(userId, callback); // Keep trying until we get something
      } else {
        var $ = cheerio.load(body),
...
var getUserFriendsGroups = function (userCookie, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    ...
    request.get(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200 || captchaTriggered(body)) {
        handleFailedRequest('getUserFriendsGroups', error, response);
        getUserFriendsGroups(userCookie, callback); // Keep trying until we get something
      } else {
        var $ = cheerio.load(body);
...
var getUserFriends = function (userCookie, groupId, callback) {
  ...
  getProxyRequestOptions(url, function (requestOptions) {
    ...
    request.post(requestOptions, function (error, response, body) {
      if (error || response.statusCode != 200) {
        handleFailedRequest('getUserFriends', error, response);
        getUserFriends(userCookie, groupId, callback); // Keep trying until we get something
      } else {
        try {
