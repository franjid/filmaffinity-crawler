...
var loadPopularFilmsPages = function(callback) {
    var url = global.parameters.filmaffinity_host + '/es/mostvisited.php';
    ...
    request.get(getRequestOptions(url), function (error, response, body) {
        if (error || response.statusCode != 200 || captchaTriggered(body)) {
            handleFailedRequest('loadFilmsPages', error, response);
            loadPopularFilmsPages(callback); // Keep trying until we get something
        }
        else
        {
            var $ = cheerio.load(body);
...
module.exports = {
    ...
    loadPopularFilmsPages: loadPopularFilmsPages,
    loadFilm: loadFilm
};
